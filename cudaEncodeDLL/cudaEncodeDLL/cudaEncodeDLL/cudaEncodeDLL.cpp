...
typedef boost::shared_ptr<CCudaVideoEncode>                       CCudaVideoEncode_ptr;
typedef boost::unordered_map<uint64_t, CCudaVideoEncode_ptr>      CCudaVideoEncodeMap;
...
bool CreateCudaVideoEncodeClient(cudaEncodeVideo_enum videoCodec, cudaEncodeVideo_enum yuvType, int nWidth, int nHeight, uint64_t& nCudaChan)
{
	...
	try
	{
		pXHClient = boost::make_shared<CCudaVideoEncode>(videoCodec, yuvType, nWidth, nHeight, ABL_nCudaVideoEncodeNumber);
	}
	...
	std::pair<boost::unordered_map<uint64_t, CCudaVideoEncode_ptr>::iterator, bool> ret =
