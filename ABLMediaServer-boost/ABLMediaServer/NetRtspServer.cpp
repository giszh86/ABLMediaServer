...
void  CNetRtspServer::InputRtspData(unsigned char* pRecvData, int nDataLength)
{
	...
	{//&& nRecvLength > 500
		...
		//判断推流地址是否存在
		boost::shared_ptr<CMediaStreamSource> pMediaSourceTemp = GetMediaStreamSource(szMediaSourceURL,true);
	...
	else if (memcmp(pRecvData, "SETUP", 5) == 0 && strstr((char*)pRecvData, "\r\n\r\n") != NULL)
	{
		...
		if(netBaseNetType == NetBaseNetType_RtspServerRecvPush)//rtsp 的tcp方式推流
