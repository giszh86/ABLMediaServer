...
int AudioRecordWas::Start()
{
    ...
    if(!_is_input) {
        ...
        _captrue_thread->detach();
        /*_captrue_thread_ptr = rtc::Thread::Create();
        _captrue_thread_ptr->Start();
        _captrue_thread_ptr->PostTask(RTC_FROM_HERE, [&]()
            {
                CaptureLoop();
            });*/
    }
    ...
    /*_record_thread_ptr = rtc::Thread::Create();
    _record_thread_ptr->Start();
    _record_thread_ptr->PostTask(RTC_FROM_HERE, [&]()
        {
            RecordLoop();
        });*/
    return AE_NO;
...
{
    RTC_LOG(LS_INFO) << "into AudioRecordWas::Stop()";
    {
        std::lock_guard<std::mutex> lock(_wakeup_mutex);
        _running = false;
...
void AudioRecordWas::CaptureLoop()
{
    ...
    while (_running && WaitForMultipleObjects(2, events, FALSE, INFINITE) != WAIT_OBJECT_0) {
        if (_bHangUp.load())
        {
            ...
            while (_running && _bHangUp.load())
            {
                _wakeup_cond.wait(lock, [=]() {
                    return !_running || _bHangUp.load() == false;
                    });
                if (!_running)
                {
                    return;
    ...
    RTC_LOG(LS_WARNING) << __FUNCTION__ << "######exit AudioRecordWas::CaptureLoop()######";
...
void AudioRecordWas::RecordLoop()
{
    ...
    RTC_LOG(LS_WARNING) << "#########exit AudioRecordWas::RecordLoop() loop#######";
...
//挂起采集
void AudioRecordWas::HangUp()
{
    std::lock_guard<std::mutex> lock(_wakeup_mutex);
    _bHangUp.store(true);
}
//唤醒采集
void AudioRecordWas::WakeUp()
{
    std::lock_guard<std::mutex> lock(_wakeup_mutex);
    _bHangUp.store(false);
    _wakeup_cond.notify_all();
}