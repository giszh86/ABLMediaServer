...
static int flvfile_read(void* param, void* buf, int len)
{
	int nRead = (int)fread(buf, 1, len, (FILE*)param);
	return nRead;
}
...
void flv_reader_test(const char* file)
{
	...
	h264 = fopen("video.264", "wb");
	FILE* fFlvFile = fopen(file, "rb");

	void* reader = flv_reader_create2(flvfile_read,fFlvFile);
...
int main(void)
{
	flv_reader_test("D:\\video\\flv\\D2428FD0.flv");
	return 0;
}