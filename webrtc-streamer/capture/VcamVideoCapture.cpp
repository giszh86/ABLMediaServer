...
VcamVideoCapture::VcamVideoCapture()
{
...
void VcamVideoCapture::Stop(VideoYuvCallBack yuvCallback)
{
	bool bEmpty = false;	
	{
...
rtc::scoped_refptr<webrtc::VideoCaptureModule> VcamVideoCapture::OpenVideoCaptureDevice()
{
...
void VcamVideoCapture::Init(const char* devicename, int nWidth, int nHeight, int nFrameRate)
{
...
void VcamVideoCapture::Init(std::map<std::string, std::string> opts)
{
...
void VcamVideoCapture::OnFrame(const webrtc::VideoFrame& video_frame)
{
...
void VcamVideoCapture::RegisterCallback(VideoYuvCallBack yuvCallback)
{
	std::lock_guard<std::mutex> _lock(m_mutex);
	std::list<VideoYuvCallBack>::iterator it = m_YuvCallbackList.begin();
	while (it != m_YuvCallbackList.end())
...
void VcamVideoCapture::StopFrameCallback(FrameCallBack frameCallback)
{
	bool bEmpty = false;
	{
		std::lock_guard<std::mutex> _lock(m_mutex);
...
void VcamVideoCapture::RegisterFrameCallback(FrameCallBack frameCallback)
{
	std::lock_guard<std::mutex> _lock(m_mutex);
