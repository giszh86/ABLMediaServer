...
static uint8_t H264_marker[] = { 0, 0, 0, 1,9, 240 , 0 , 0 , 0 , 1 };
...
public:
	...
	std::vector< std::vector<uint8_t> > getInitFrames(const std::string& codec, const char* sdp) {
		...
		if (codec == "H264") {
			...
			if (sprop)
			{
				...
				{
					SPDLOG_LOGGER_INFO(spdlogptr, "Cannot decode SPS:{}", sprop);
				}
			}
		}
		else if (codec == "H265") {
	...
	virtual int32_t Decoded(webrtc::VideoFrame& decodedImage) override {
		...
		SPDLOG_LOGGER_TRACE(spdlogptr, "VideoDecoder::Decoded size:{}, decode ts:{}   source ts:{}",
			decodedImage.size(), decodedImage.ntp_time_ms(), ts);
	...
	void DecoderThread()
	{
		while (!m_stop) {
			...
			if (frame.m_content.get() != NULL) {
				...
				auto size = frame.m_content->size();
				...
				if (size) {
					...
					if (this->hasDecoder()) {
						...
						SPDLOG_LOGGER_ERROR(spdlogptr, "VideoDecoder::DecoderThread no decoder");
					}
				}
			}
		}
