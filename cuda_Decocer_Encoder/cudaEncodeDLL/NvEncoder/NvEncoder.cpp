...
int NvEncoder::EncodeFrame(uint8_t* data, int max, uint32_t& flags, uint64_t& timestap, NV_ENC_PIC_PARAMS *pPicParams)
{
    if (!IsHWEncoderInitialized())
    {
        NVENC_THROW_ERROR("Encoder device not found", NV_ENC_ERR_NO_ENCODE_DEVICE);
    ...
    return DoEncode(m_vMappedInputBuffers[i], data, max, flags, timestap, pPicParams);
}
...
int NvEncoder::DoEncode(NV_ENC_INPUT_PTR inputBuffer, uint8_t* data, int max, uint32_t& flags, uint64_t& timestap, NV_ENC_PIC_PARAMS *pPicParams)
{
    int ret = -1;
    NV_ENC_PIC_PARAMS picParams = {};
    if (pPicParams)
    ...
    {
        m_iToSend++;
        ret = GetEncodedPacket(m_vBitstreamOutputBuffer, data, max, flags, timestap, true);
    }
    else
    {
        NVENC_THROW_ERROR("nvEncEncodePicture API failed", nvStatus);
...
int NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer, uint8_t* data, int max, uint32_t& flags, uint64_t& timestap, bool bOutputDelay)
{
    ...
    for (; m_iGot < iEnd; m_iGot++)
    {
        ...
        else if (lockBitstreamData.pictureType == NV_ENC_PIC_TYPE_I)
        {
           flags |= 0x40000000;
        }
        timestap = lockBitstreamData.outputTimeStamp;

        //不要越界
        if (offset + lockBitstreamData.bitstreamSizeInBytes < max)
        {
            memcpy(data + offset, lockBitstreamData.bitstreamBufferPtr, lockBitstreamData.bitstreamSizeInBytes);
            offset += lockBitstreamData.bitstreamSizeInBytes;
        }
